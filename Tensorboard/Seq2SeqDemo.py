...
class Seq2SeqEncoder(d2l.Encoder):
    ...
    def forward(self, X, *args):
        ...
        X = self.embedding(X)
        print("after embedding X's shape is :",X.shape)
...
class Seq2SeqDecoder(d2l.Decoder):
    ...
    def forward(self, X, state):
        # 输出'X'的形状：(batch_size,num_steps,embed_size)
        print("before chage X shape is :",X.shape)
        X = self.embedding(X).permute(1, 0, 2)
        print("After chage X shape is :", X.shape)
        # 广播context，使其具有与X相同的num_steps
        context = state[-1].repeat(X.shape[0], 1, 1)
        print("context shape is :", context.shape)
        X_and_context = torch.cat((X, context), 2)
        print("X_and_context shape is :", X_and_context.shape)
...
def train():
    ...
    # below is to use the network to train the data
